...
using PlutoUI, SymPy, Plots, QuadGK, Base.MathConstants

# ╔═╡ a3027df9-c1c2-4ada-a6d7-92622ad1e685
plotly()

# ╔═╡ 55b5d0c9-d968-4bb2-8a77-8a52d2e0ead0
...
__Pitanje.__ Možemo li slično napraviti i za regularni SLP, odnosno možemo li naći inverzni operator  $L^{-1}$ tako da vrijedi (1)? 
...
Greenova funkcija je __neprekidna__: $u_1$, $u_2$ i njihove derivacije su neprekidne funkcije po pretpostavci. Funkcija $p$ je neprekidna i različita od nule, a Wronskijan je neprekidan i različit od nule jer su funkcije $u_1$ i $u_2$ linearno nezavisne. Posebno, Greenova funkcija je očito neprekidna u točki $x=\xi$ (limesi s lijeva i zdesna su jednaki!).
...
# ξ=0.5
g(x,ξ)=x*(1-ξ)*Heaviside(ξ-x)+ξ*(1-x)*Heaviside(x-ξ)
...
# ╔═╡ 954059ba-2fe4-48bf-bb10-b6e74e3969ca
# Rješenje pomoću Greenove funkcije
f(x)=sin(x)

# ╔═╡ e3a85dc2-2c90-4f2e-ae6b-921f0a5e125a
...
$$
u(x) = \int_0^1 g(x,\xi)\,f(\xi)\,d\xi+B \tag{4}$$
...
$$
u(x)=\int_0^1 \delta(\xi-x)\, u(\xi)\, d\xi. \tag{5}$$

Uvrštavanjem DJ u (4) imamo

$$
u(x)=\int_0^1 g(x,\xi)\,\frac{d}{d\xi}\bigg(\frac{d}{d\xi}+1\bigg)\,u(\xi)\, 
d\xi + B$$
...
Uvrštavanjem i korištenjem (5) imamo
...
\frac{\partial g}{\partial \xi}=\delta(\xi-x) \tag{6} $$

$$
B=\bigg[\frac{\partial g}{\partial \xi} u-
g\bigg(\frac{du}{d\xi}+u\bigg)\bigg]_0^1
\tag{7}$$
"""
...
$$
g(x,0)=0,\quad g(x,1)=0, \tag{8}$$

dok ćemo vrijednosti $g_\xi(x,0)$ i $g_\xi(x,1)$ izračunati.

Integriranje (6) po $\xi$ daje (derivacija Heavisideove funkcije je delta funkcija)
...
# ╔═╡ 6d7c95f2-96f2-44f9-ae2f-d68718033ec4
md"""
...
\left( |e^{-x}-e^{-\xi}|+e^{-x}-e^{-\xi}\right).\end{aligned}$$
...
$$
g(x,\xi)=\frac{1-e^{1-x}}{e-1}(e^\xi-1)+\frac{1}{2} e^\xi\left( |e^{-x}-e^{-\xi}|+e^{-x}-e^{-\xi}\right). \tag{10}$$

Da bismo odredili $B$ iz (7) trebamo još izračunati $g_\xi(x,0)$ i 
$g_\xi(x,1)$. Vrijedi
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "3c6158745ab940ae3018a03c978f41fa8a546854"
