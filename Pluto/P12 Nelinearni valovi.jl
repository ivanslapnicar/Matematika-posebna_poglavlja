...
# ╔═╡ 123d0643-53c2-4c99-be75-112178882e66
using PlutoUI, Plots

# ╔═╡ f2669ff7-55ad-433e-80ec-1ed64630e60a
plotly()

# ╔═╡ 1cb40440-fd1b-47b0-831c-b038dfca86d0
...
$$
\begin{aligned}
&u_t+c(u)\, u_x=0, \quad x\in\mathbb{R}, \quad t>0,\quad c'(u)>0\qquad (1) \\
&u(x,0)=\phi(x), \quad x\in \mathbb{R},\end{aligned}$$
...
# ╔═╡ 5681a7e9-4ec1-41a4-8288-c20f19ca60f0
md"""

$$
\frac{du(x(t),t)}{dt}=u_x\frac{dx(t)}{dt}+u_t\frac{dt}{dt} =u_x\cdot c(u)+ u_t=0. \tag{3}$$
...
md"""
Vidimo da rješenje nakon $t=1$ __ne postoji__ (val se prelomi). Na primjer, točka $(x,t)=(4,2)$ se nalazi na dvije različite karakteristike i to $x=2t$ (za $\xi=0$) i $x=t+2$ (za $\xi=2$). Uzduž prve karakteristike sve točke imaju vrijednost $u=\phi(0)=2$, a uzduž druge karakteristike sve točke imaju vrijednost $u=\phi(2)=1$. Stoga bi vrijednost $u(4,2)$ trebala imati dvije različite vrijednosti, što je nemoguće.
...
Deriviranje jednakosti (5), $x=x(t)=c(\phi(\xi))\,t+\xi$, po $x$ daje
...
Deriviranje jednakosti (4), $u(x,t)=\phi(\xi)$, po $x$ daje
...
poprima minimum za $\xi=\sqrt{\displaystyle\frac{1}{2}}$ pa je vrijeme loma $t_b=\sqrt{\displaystyle\frac{e}{2}}\approx 1.166$.
"""
...
begin
	...
	A=1
	k=1
	Xᵦ=range(-pi,stop=pi,length=101)
	Tᵦ=range(0,stop=10,length=121)
	uᵦ(x,t)=A*exp(-ν*k^2*t)*cos(k*(x-c*t))
	surface(Xᵦ,Tᵦ,uᵦ,xlabel="x",ylabel="t",zlabel="u")
end
