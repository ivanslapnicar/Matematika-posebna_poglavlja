...
# ╔═╡ d0e1ade5-bf6a-4c0e-a798-57ac28c88c78
using PlutoUI, Plots

# ╔═╡ 4cad5e67-1d18-4334-8ff4-50e202c1b149
plotly()

# ╔═╡ 978f5fc9-d5a6-463e-bb82-155d8d542217
TableOfContents(title="📚 Sadržaj", aside=true)

# ╔═╡ 483ce4b3-7a51-4486-86d8-ef79fcbcafa8
md"""
# Valovi
...
je __desni val__, odnosno val koji putuje udesno brzinom $c$. 
"""

# ╔═╡ eebbe978-0093-4958-bcee-1db643d36a58
@bind t₀ Slider(0:0.5:5,show_value=true)

# ╔═╡ 52ec7704-1988-4129-925e-3acc53c923a6
begin
	c=1
	f₁(x)=(x-c*t₀)^2
end

# ╔═╡ ae797793-3175-41ce-9a3d-314c4524475d
plot(f₁,ylim=(0,10),legend=false)

# ╔═╡ da811f07-9dda-4191-813e-4d11aa3b7357
md"""

## Advekcijska jednadžba
...
Dakle, ako u trenutku $t=0$ poremećaj ima profil $u(x,0)=f(x)$ (početni uvjet), onda se u trenutku $t>0$ poremećaj pomaknu udesno za $ct$ jedinica duljine.
...
begin
	...
	s=1.7
	v(x)=sin(π*(x-s))
	plot(x->x,[u,v],0,2*π,xlabel="x",ylabel="u",aspect_ratio=1, label=["u" "v"])
end
...
begin
	...
	t₃=1.6
	u₃(x)=sin(pi*x)*cos(pi*t₃)
	plot(x->x,[u₁,u₂,u₃],0,2*pi,xlabel="x",ylabel="u",label=["u₁" "u₂" "u₃"])
end
...
begin
	...
	T=range(0,stop=4,length=101)
	u(x,t)=sin(pi*x)*cos(pi*t)
	surface(X,T,u,xlabel="x",ylabel="t",seriescolor=:blues,aspect_ratio=1)
end
...
begin
	...
	ω₁=0.8
	u₄(x)=A₁*cos(k₁*x-ω₁*t₄)
	plot(u₄,0,4*pi,xlabel="x",ylabel="u",legend=false)
end
...
begin
	...
	ω₂=0.8
	u₅(t)=A₂*cos(k₂*x₁-ω₂*t)
	plot(u₅,0,4*pi,xlabel="t",ylabel="u",legend=false)
end
...
te promatrati realni ili imaginarni dio kako bi se dobilo realno rješenje. Uvrštavanje Eulerovog oblika u parcijalnu diferencijalnu jednadžbu daje nam __relaciju disperzije__, odnosno vezu između valnog broja i kutne frekvencije,
...
Faktor $e^{ik(x-\gamma t)}$ je desni val s valnim brojem $k$, a faktor $\displaystyle e^{-Dk^2t}$ je opadajuća amplituda (gušenje). Zaključujemo sljedeće:

* za konstantni valni broj $k$ atenuacija (opadanje) vala se povećava s $D$ pa je $D$ mjera difuzije,
* za konstantnu vrijednost $\gamma$, atenuacija se povećava kada $k$ raste, što znači da valovi s manjim valnim duljinama opadaju brže od valova s većim valnim duljinama ("mrtvo more").
...
jednadžba hiperbolična. 

> Vidimo da vibracije s većim valnim brojem, odnosno manjom valnom duljinom $2\pi /k$ putuju brže.
...
Brzina kojom se val kreće (kojom se početne vrijednosti propagiraju uzduž katakterističnih krivulja) dana je s (2).
"""
