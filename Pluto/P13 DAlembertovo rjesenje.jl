...
# ╔═╡ d37674db-05ec-4cfb-9054-d35b2c05e385
using PlutoUI, Plots, QuadGK

# ╔═╡ 844eb74f-dd42-4b72-b36d-80b7df50e0ac
plotly()

# ╔═╡ f07c9639-08fc-4f3a-96ea-2cccd293d1c5
...
$$
\begin{aligned}
u(x,0)&=f(x)+g(x)=F(x), \qquad \qquad \qquad (1)\\
u_t(x,0)&=f'(x)\cdot c+g'(x)\cdot (-c)=G(x),\end{aligned}$$
...
begin
	...
	surface(X,T,u,xlabel="x",ylabel="y",zlabel="z",aspect_ratio=1)
end
...
Obrnuto, neka za $u(x,t)$ vrijedi (4). Onda za $h=0$ vrijedi
...
pa uvrštavanje u (5), dijeljenje s $k^2$ i ignoriranje članova $O(k)$ daje (3) i lema je dokazana.
...
begin
	...
	Labels = [("A",:top),("B",:left),("C",:bottom),("D",:right),("(x,t)",:top)]
	plot(x->(x-1)/c,1,1.75)
	plot!(x->(x-0)/c,0,1.25)
	plot!(x->(x-2)/(-c),0.75,2)
	plot!(x->(x-1.5)/(-c),0.5,1.5)
	scatter!(X₁,T₁,label=false,ms=0,series_annotations=Labels, legend=false,
...
begin
	...
	Labels₂ = [("S",:right),("P",:top),("Q",:left),("R",:bottom),("K",:bottom),
		("N",:bottom),("M",:top),("L",:left)]
...
Primijenimo postupak na poluomeđenu domenu: 
...
U sustavu $(\xi,y)$ koji se preklapa sa sustavom $(x,t)$, točka $S$ leži na pravcu 
...
Dakle, u sustavu $(\xi,y)$ je $S=\big(0,t-\frac{x}{c}\big)$ pa iz početnog uvjeta slijedi
...
begin
	...
	u₃(x,t)= t<=x/c₃ ? (f(x+c₃*t)+f(x-c₃*t))/2+quadgk(g,x-c₃*t,x+c₃*t)[1]/(2*c₃) : h(t-x/c₃)+(f(x+c₃*t)-f(c₃*t-x))/2+quadgk(g,c₃*t-x,x+c₃*t)[1]/(2*c₃)
	surface(X₃,T₃,u₃, xlabel="x", ylabel="t", zlabel="u")
end

# ╔═╡ a751041b-a370-4cb9-9500-8e461ee27676
# Provjerimo početni uvjet
plot(X₃,u₃.(X₃,0),xlabel="x",title="Početni uvjet",legend=false)

# ╔═╡ 42fb3476-49cb-44a5-abbc-389fb9a122a2
# Provjerimo rubni uvjet
plot(T₃,u₃.(0,T₃),xlabel="t",title="Rubni uvjet",legend=false)
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "a726e2ec43a9d65f867e0be04b71730dc3051ace"
